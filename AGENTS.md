...
## 三十、Android 真机测试与安装安全规则

本规则长期适用于所有版本。任何涉及 Android 真机安装、集成测试、验收测试、Debug/Release APK 安装或 `flutter run` 的任务，在执行可能安装或替换应用的命令之前，必须先检查目标设备上现有的 MediaFlow 安装情况，并确认：

- 设备上是否已安装 MediaFlow，以及已安装应用的 package/applicationId；
- 本次测试包的 package/applicationId；
- 已安装应用与测试包的签名是否兼容；
- 安装命令或测试工具是否可能自动卸载、覆盖、替换、清除数据，或导致现有应用数据丢失。

默认原则：

1. 不得为了运行测试而自动卸载设备上已有的 MediaFlow。
2. 未明确确认现有应用及其数据安全之前，不得执行可能覆盖、替换或清除现有安装的操作。
3. Debug 与 Release 签名不兼容时，禁止通过先卸载原应用再安装 Debug 包继续测试；也须防止 Flutter、ADB 或其他测试工具自动执行这一路径。
4. 优先选择不影响现有正式安装的方案，例如独立测试 applicationId / applicationIdSuffix、独立测试包、无需安装 APK 的测试方式，或其他隔离测试数据与正式数据的方法。
5. 当前测试架构若无法避免覆盖或卸载现有应用，应停止安装步骤，在报告中写明阻断原因和推荐方案，不得自行删除现有应用。
6. `adb uninstall`、清除应用数据、卸载后重装，以及其他可能造成用户数据丢失的操作均属破坏性操作；未经用户明确授权不得执行。
7. 真机验收完成后，必须报告实际使用的 applicationId、Debug/Release/Test 包类型、是否与已有安装共存、是否发生覆盖/卸载/数据清除，以及是否发现签名冲突。未能核实的项目须明确标为未核实。
